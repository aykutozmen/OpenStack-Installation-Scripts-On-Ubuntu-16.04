...
echo " | If you don't have ISO file downloaded this script will be download it.                                              |"
echo " | ISO file name should be like 'ubuntu-16.04.4-server-amd64.iso'                                                      |"
echo " | ISO file should be in the directory where this scripts is running in.                                               |"
echo " | This script must be run with maximum privileges. Run with sudo or run it as 'root'.                                 |"
echo " | Seed file will be downloaded from https://raw.githubusercontent.com/netson/ubuntu-unattended/master/netson.seed     |"
echo " | If you have a seed file, locate it to the directory where this script is running with filename 'netson.seed'        |"
echo " +---------------------------------------------------------------------------------------------------------------------+"
echo
echo " +---------------------------------------------------+"
...
# ask if script runs without sudo or root privileges
...
read -ep " please enter your preferred username: " -i "openstack" username
...
cd $tmp/iso_new
(mkisofs -D -r -V "UNATTANDED_UBUNTU" -cache-inodes -J -l -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -o $tmp/$new_iso_name . > /dev/null 2>&1) &
spinner $!
